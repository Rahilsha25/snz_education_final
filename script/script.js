...
if (contactForm) {
  contactForm.addEventListener('submit', function(e) {
    e.preventDefault(); // Temporarily stop default submit to validate first
    
    const name = document.getElementById('name').value.trim();
    const phone = document.getElementById('phone').value.trim(); // Updated to phone
    ...
    // Mobile validation (Basic format check for numbers)
    ...
    // IF VALID -> AJAX Submission to Formspree
    if (isValid) {
      formMessage.className = 'mt-4 p-4 bg-blue-100 border border-blue-400 text-blue-700 rounded-lg';
      formMessage.textContent = 'Sending message...';

      // Send the data background-style using Fetch API to Formspree
      ...
      .catch(error => {
        ...
        formMessage.textContent = 'Oops! There was a problem sending your form. Please try again.';
      });

    } else {
      // IF INVALID -> Show frontend errors
      formMessage.className = 'mt-4 p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg';
